...
def run_driver_with_io(comm: MPI.Comm, config: MPCConfig, input_path: str):
    ...
    for phase in range(8): # Fixed rounds for demo
        # Check active
        ...
        # 1. Sparsify
        p_val = 0.5 # Fixed probability for stability
    ...
    if rank == 0:
        final = []
        for l in all_lists: final.extend(l)
        print(f"Algorithm Complete. Total Matching: {len(final)}")