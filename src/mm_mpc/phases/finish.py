...
def finish_small_components(comm: MPI.Comm, edge_state: EdgeState, config: MPCConfig) -> list:
    rank = comm.Get_rank()
    active = np.where(edge_state.active_mask)[0]
    my_edges = edge_state.edges_local[active]
    ...
    recv_buf = None
    if rank == 0: recv_buf = np.empty(np.sum(recv_counts), dtype=np.int64)
    
    displs = None
    if rank == 0: displs = np.concatenate(([0], np.cumsum(recv_counts)[:-1]))
